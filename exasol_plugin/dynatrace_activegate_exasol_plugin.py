...
class ExasolPluginRemote(RemoteBasePlugin):
    ...
    def getIdleSessions(self,db,device):
        sqlCommand = "select count(*) from sys.exa_all_sessions WHERE STATUS = 'IDLE';"
        ...
        if not None in result:
            sessions = {
                MetricPoint(key="sessions.idle", value=result[0])
            }
            self.reportAbsolute(device,sessions)


    def getBackupInterval(self,db,device):
        sqlCommand = "select SECONDS_BETWEEN(SYSTIMESTAMP,max(measure_time))/60/60 from EXA_SYSTEM_EVENTS where event_type='BACKUP_END' limit 1;"
        ...
        if not None in result:
            sessions = {
                MetricPoint(key="backup.interval", value=result[0])
            }
